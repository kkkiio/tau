...
Pi Web UI 需要支持"编辑历史用户消息并重新发送"功能，允许用户在 Web UI 中点 Edit 按钮修改一条之前的用户消息，从该点分叉出新对话分支。
...
Pi Web UI 的核心事件处理（WebSocket handler）运行在 `ExtensionContext` 中，因此无法直接调用 `navigateTree`。
...
- Pi Web UI `mirror-server.ts`: two `session_start` handlers (auto-title at ~line 585, auto-start at ~line 2042). Clearing logic should go in the auto-title handler since it already manages session-level state.
